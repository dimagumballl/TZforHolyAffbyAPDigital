...
(function () {
	...
	if (aCurrentId.length != 0) {
		...
		var checkBrowserNavigatorLanguage = navigator.language || navigator.browserLanguage; // By IE10 and older Browser
		if (checkBrowserNavigatorLanguage == undefined) {
			checkBrowserNavigatorLanguage = window.navigator.language;
		}
        var get_browser_language = checkBrowserNavigatorLanguage.toLocaleLowerCase().substring(0,2);
		...
		var selectedForm = d.getElementById('form' + aCurrentId[0]) || d.getElementById('form_reg_both');

		/* avoid js Error if form id != form_reg_half || form_reg_full || form_reg_both */
		if (selectedForm != null) {
            selectedForm.appendChild(fieldBfp);
			ab.setHF('bfp', createHash());

            if (get_browser_language !== undefined && selectedForm.id.match("form_reg_both|form_reg_full")) {
	...
	function createHash() {
		...
		function plugInsTest() {
			...
			for (i = 0; i < l; i++) {
				var output = navP[i].name + '|' + navP[i].description + '|' + navP[i].filename;
				readHashData(output);
			}
		};
		...
		function mimeTypesTest() {
			...
			for (i = 0; i < l; i++) {
				var output = navMT[i].type + '|' + navMT[i].suffixes + '|' + navMT[i].description + '|' + testEnabled(i);
				readHashData(output);
			}
		};
